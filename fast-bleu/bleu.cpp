...
vector<vector<double>> BLEU_CPP::get_score(vector<vector<string>> hypotheses)
{
    ...
    int hypotheses_size = (int)hypotheses.size();
    // cout << "calculating bleu_0_" << endl;
    for (vector<float> &w : this->weights)
    {
        // double temp_results[hypotheses.size()];
        double *temp_results =  new double[hypotheses_size];
        int curr_n = w.size();
        // cout << "calculating bleu_1_" << curr_n << " __" << hypotheses_size << " scores!" << endl;

#pragma omp parallel
        {
            // cout << "calculating bleu_2____" << "_scores!" << endl;
#pragma omp for schedule(guided)
            // cout << "calculating bleu_3____" << "_scores!" << endl;
            for (int i = 0; i < hypotheses_size; i++)
            {
                // cout << "calculating bleu_3___1____" << i << "_scores!" << endl;
                vector<string> *hypothesis = &(hypotheses[i]);
                // cout << "calculating bleu_3___2____" << "_scores!" << endl;
                temp_results[i] = corpus_bleu(number_of_refs, 
                                              curr_n,
                                              references,
                                              hypothesis,
                                              ...
                                              smoothing_function,
                                              auto_reweigh);
                // cout << "calculating bleu_3___3____" << "_scores!" << endl;
            }
        }
        // cout << "calculating bleu_4_" <<"_scores!" << endl;
        results.push_back(vector<double>());
        // cout << "calculating bleu_5_" << " scores!" << endl;
        for (int i = 0; i < hypotheses_size; i++) {
            results.back().push_back(temp_results[i]);
        }
        // cout << "calculating bleu_6_"  << " scores!" << endl;
        delete[] temp_results;
        // cout << "calculating bleu_7_" << " scores!" << endl;
    }
    // cout << "calculating bleu_9_" << " scores!" << endl;
    return results;
}